...
public class AuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        Optional<Cookie> tokenContainedCookie = Arrays.asList(request.getCookies())
                .parallelStream()
                .filter(cookie -> COOKIE_NAME.equals(cookie.getName()) &&
                        cookie.getValue() != null &&
                        cookie.getValue().startsWith(TOKEN_PREFIX))
                .findAny()
                ;
    ...
    private void extractUserId(Cookie cookie, HttpServletRequest request) {
        String id = null;
        final String jwtToken = cookie.getValue().substring(TOKEN_PREFIX.length());
        try {
